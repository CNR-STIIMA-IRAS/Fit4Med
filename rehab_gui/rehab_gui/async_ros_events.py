...
class ASyncRosManager(QObject):
    ...
    def __init__(self, ros_node, joint_names, controller_name='joint_trajectory_controller'):
        ...
        self.clientFollowCartTraj = ActionClient(self._node, FollowJointTrajectory, f'/{self.controller_name}/follow_joint_trajectory')
        if not self.clientFollowCartTraj.wait_for_server(2):
            print("Follow Joint Trajectory Action server not available, exiting...")
            rclpy.shutdown()
            sys.exit(1)
        self.clientSetMOOAsync = ActionClient(self._node, SetModesOfOperationAction, '/state_controller/async_set_modes_of_operation')
        if not self.clientSetMOOAsync.wait_for_server(2):
            print("Set Modes of Operation Action server not available, exiting...")
        self.clearFCT() 
    ...
    def start_movementFCT(self, default_speed_ovr):
        ...
        self._pause_start_time = None
        self.goalFCT.trajectory.header.stamp = self._node.get_clock().now().to_msg()
        self._send_goal_future = self.clientFollowCartTraj.send_goal_async(self.goalFCT)
    ...
    def change_mode_of_op(self, dof, new_mode):
        self.mooGoal = SetModesOfOperationAction.Goal()
        self.mooGoal.dof_name = dof
        self.mooGoal.mode_of_operation = new_mode
        self.moo_future = self.clientSetMOOAsync.send_goal_async(self.mooGoal)
        self.moo_future.add_done_callback(self.accepted_mode_of_op)

    def accepted_mode_of_op(self, future):
        self.moo_goal_handle = future.result()
        if not self.moo_goal_handle.accepted:
            self._node.get_logger().info('Goal rejected!!')
            return
        self._node.get_logger().info('Goal accepted')
        self._get_moo_goal_result_future = self.moo_goal_handle.get_result_async()
        self._get_moo_goal_result_future.add_done_callback(self.moo_done_callback)

    def moo_done_callback(self, future):
        self.moo_finished.emit()
